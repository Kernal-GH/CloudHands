...
/*****************************************************************
 * This file contains array and ch_table_t functions only.
 */
...
static void make_array_core(ch_array_header_t *res, ch_pool_t *p,
			    int nelts, int elt_size, int clear)
{
    ...
    if (clear) {
        res->elts = ch_pcalloc(p, nelts * elt_size);
    }
    else {
        res->elts = ch_palloc(p, nelts * elt_size);
    }
...
int ch_is_empty_array(const ch_array_header_t *a)
{
    return ((a == NULL) || (a->nelts == 0));
}

ch_array_header_t * ch_array_make(ch_pool_t *p,
						int nelts, int elt_size)
{
    ch_array_header_t *res;

    res = (ch_array_header_t *) ch_palloc(p, sizeof(ch_array_header_t));
    make_array_core(res, p, nelts, elt_size, 1);
    return res;
}

void ch_array_clear(ch_array_header_t *arr)
{
    arr->nelts = 0;
}

void * ch_array_pop(ch_array_header_t *arr)
{
    if (ch_is_empty_array(arr)) {
        return NULL;
    }
...
void * ch_array_push(ch_array_header_t *arr)
{
    if (arr->nelts == arr->nalloc) {
        int new_size = (arr->nalloc <= 0) ? 1 : arr->nalloc * 2;
        char *new_data;

        new_data = ch_palloc(arr->pool, arr->elt_size * new_size);
...
static void *ch_array_push_noclear(ch_array_header_t *arr)
{
    if (arr->nelts == arr->nalloc) {
        int new_size = (arr->nalloc <= 0) ? 1 : arr->nalloc * 2;
        char *new_data;
...
void ch_array_cat(ch_array_header_t *dst,
			       const ch_array_header_t *src)
{
	...
	new_data = ch_pcalloc(dst->pool, elt_size * new_size);
...
ch_array_header_t * ch_array_copy(ch_pool_t *p,
						const ch_array_header_t *arr)
{
    ch_array_header_t *res =
        (ch_array_header_t *) ch_palloc(p, sizeof(ch_array_header_t));
...
ch_array_header_t *
    ch_array_append(ch_pool_t *p,
		      const ch_array_header_t *first,
		      const ch_array_header_t *second)
{
    ch_array_header_t *res = ch_array_copy_hdr(p, first);

    ch_array_cat(res, second);
    return res;
}

/* ch_array_pstrcat generates a new string from the ch_pool_t containing
 * the concatenated sequence of substrings referenced as elements within
 * the array.  The string will be empty if all substrings are empty or null,
 * or if there are no elements in the array.
 * If sep is non-NUL, it will be inserted between elements as a separator.
 */
char * ch_array_pstrcat(ch_pool_t *p,
				     const ch_array_header_t *arr,
				     const char sep)
{
    char *cp, *res, **strpp;
...
struct ch_table_t {
     ...
     * older modules which cast a ch_table_t * to an ch_array_header_t *...
     * they should use the ch_table_elts() function for most of the
...
/* keep state for ch_table_getm() */
typedef struct
{
    ch_pool_t *p;
    const char *first;
    ch_array_header_t *merged;
} table_getm_t;
 ...
 */
#ifdef MAKE_TABLE_PROFILE
static ch_table_entry_t *do_table_push(const char *func, ch_table_t *t)
{
    if (t->a.nelts == t->a.nalloc) {
        fprintf(stderr, "%s: table created by %p hit limit of %u\n",
                func ? func : "table_push", t->creator, t->a.nalloc);
    }
    return (ch_table_entry_t *) ch_array_push_noclear(&t->a);
}
...
#define table_push(t)	((ch_table_entry_t *) ch_array_push_noclear(&(t)->a))
#endif /* MAKE_TABLE_PROFILE */

const ch_array_header_t * ch_table_elts(const ch_table_t *t)
{
    return (const ch_array_header_t *)t;
}

int ch_is_empty_table(const ch_table_t *t)
{
    return ((t == NULL) || (t->a.nelts == 0));
}

ch_table_t * ch_table_make(ch_pool_t *p, int nelts)
{
    ch_table_t *t = ch_palloc(p, sizeof(ch_table_t));

    make_array_core(&t->a, p, nelts, sizeof(ch_table_entry_t), 0);
#ifdef MAKE_TABLE_PROFILE
...
ch_table_t * ch_table_copy(ch_pool_t *p, const ch_table_t *t)
{
    ch_table_t *new = ch_palloc(p, sizeof(ch_table_t));


    make_array_core(&new->a, p, t->a.nalloc, sizeof(ch_table_entry_t), 0);
    memcpy(new->a.elts, t->a.elts, t->a.nelts * sizeof(ch_table_entry_t));
...
ch_table_t * ch_table_clone(ch_pool_t *p, const ch_table_t *t)
{
    const ch_array_header_t *array = ch_table_elts(t);
    ch_table_entry_t *elts = (ch_table_entry_t *) array->elts;
    ch_table_t *new = ch_table_make(p, array->nelts);
    int i;

    for (i = 0; i < array->nelts; i++) {
        ch_table_add(new, elts[i].key, elts[i].val);
    }

    return new;
}

static void table_reindex(ch_table_t *t)
{
    int i;
    int hash;
    ch_table_entry_t *next_elt = (ch_table_entry_t *) t->a.elts;
...
void ch_table_clear(ch_table_t *t)
{
    t->a.nelts = 0;
    t->index_initialized = 0;
}

const char * ch_table_get(const ch_table_t *t, const char *key)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
    uint32_t checksum;
    int hash;
    ...
    next_elt = ((ch_table_entry_t *) t->a.elts) + t->index_first[hash];;
    end_elt = ((ch_table_entry_t *) t->a.elts) + t->index_last[hash];
...
void ch_table_set(ch_table_t *t, const char *key,
                                const char *val)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
    ch_table_entry_t *table_end;
    uint32_t checksum;
    int hash;
    ...
    next_elt = ((ch_table_entry_t *) t->a.elts) + t->index_first[hash];;
    end_elt = ((ch_table_entry_t *) t->a.elts) + t->index_last[hash];
    table_end =((ch_table_entry_t *) t->a.elts) + t->a.nelts;
	...
	if ((checksum == next_elt->key_checksum) &&
            ...
            int must_reindex = 0;
            ch_table_entry_t *dst_elt = NULL;

            next_elt->val = ch_pstrdup(t->a.pool, val);
...
add_new_elt:
    ...
    next_elt = (ch_table_entry_t *) table_push(t);
    next_elt->key = ch_pstrdup(t->a.pool, key);
    next_elt->val = ch_pstrdup(t->a.pool, val);
    next_elt->key_checksum = checksum;
}

void ch_table_setn(ch_table_t *t, const char *key,
                                 const char *val)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
...
void ch_table_unset(ch_table_t *t, const char *key)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
    ch_table_entry_t *dst_elt;
    uint32_t checksum;
    int hash;
    int must_reindex;
	...
	if ((checksum == next_elt->key_checksum) &&
             ...
             */
            ch_table_entry_t *table_end = ((ch_table_entry_t *) t->a.elts) +
                t->a.nelts;
            t->a.nelts--;
            dst_elt = next_elt;
            for (next_elt++; next_elt <= end_elt; next_elt++) {
...
void ch_table_merge(ch_table_t *t, const char *key,
				 const char *val)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
	...
	if ((checksum == next_elt->key_checksum) &&
	    ...
	    next_elt->val = ch_pstrcat(t->a.pool, next_elt->val, ", ",
                                        val, NULL);
            return;
        }
    }
...
void ch_table_mergen(ch_table_t *t, const char *key,
				  const char *val)
{
    ch_table_entry_t *next_elt;
    ch_table_entry_t *end_elt;
...
void ch_table_add(ch_table_t *t, const char *key,
			       const char *val)
{
    ch_table_entry_t *elts;
    uint32_t checksum;
    int hash;
    ...
    COMPUTE_KEY_CHECKSUM(key, checksum);
    elts = (ch_table_entry_t *) table_push(t);
    elts->key = ch_pstrdup(t->a.pool, key);
    elts->val = ch_pstrdup(t->a.pool, val);
    elts->key_checksum = checksum;
}

void ch_table_addn(ch_table_t *t, const char *key,
				const char *val)
{
    ch_table_entry_t *elts;
    uint32_t checksum;
...
ch_table_t * ch_table_overlay(ch_pool_t *p,
					     ...
					     const ch_table_t *base)
{
    ...
    res = ch_palloc(p, sizeof(ch_table_t));
    /* behave like append_arrays */
    res->a.pool = p;
    copy_array_hdr_core(&res->a, &overlay->a);
    ch_array_cat(&res->a, &base->a);
    table_reindex(res);
    return res;
}
 ...
 *   on each valid key-value pair in the ch_table_t t that matches the vararg key,
 ...
 * the ch_table_t is unique (i.e., no multiple entries with the same key).  This
 ...
 *
 * ADDENDUM for ch_table_vdo():
 * 
 * The caching api will allow a user to walk the header values:
 *
 * ch_status_t ch_cache_el_header_walk(ch_cache_el *el, 
 *    int (*comp)(void *, const char *, const char *), void *rec, ...);
 *
 * So it can be ..., however from there I use a  callback that use a va_list:
 *
 * ch_status_t (*cache_el_header_walk)(ch_cache_el *el, 
 *    int (*comp)(void *, const char *, const char *), void *rec, va_list);
 *
 * To pass those ...'s on down to the actual module that will handle walking
 * their headers, in the file case this is actually just an ch_table - and
 * rather than reimplementing ch_table_do (which IMHO would be bad) I just
 * called it with the va_list. For mod_shmem_cache I don't need it since I
 * can't use ch_table's, but mod_file_cache should (though a good hash would
 ...
 * the entire ch_table_vdo (which is what I think should happen in
 * both cases).
 *
 ...
 * function will cause a full termination of ch_table_vdo.  I'm hesitant
 ...
 * Sigh.  --JCW, 06/28/02
 */
int ch_table_vdo(ch_table_do_callback_fn_t *comp,
                               void *rec, const ch_table_t *t, va_list vp)
{
    char *argp;
    ch_table_entry_t *elts = (ch_table_entry_t *) t->a.elts;
    int vdorv = 1;
...
static ch_table_entry_t **table_mergesort(ch_pool_t *pool,
                                           ch_table_entry_t **values, 
                                           size_t n)
{
    /* Bottom-up mergesort, based on design in Sedgewick's "Algorithms
     * in C," chapter 8
     */
    ch_table_entry_t **values_tmp =
        (ch_table_entry_t **)ch_palloc(pool, n * sizeof(ch_table_entry_t*));
    size_t i;
    size_t blocksize;
    ...
    for (i = 0; i + 1 < n; i += 2) {
        if (strcasecmp(values[i]->key, values[i + 1]->key) > 0) {
            ch_table_entry_t *swap = values[i];
            values[i] = values[i + 1];
            values[i + 1] = swap;
        }
    }
    ...
    blocksize = 2;
    while (blocksize < n) {
        ch_table_entry_t **dst = values_tmp;
        size_t next_start;
        ch_table_entry_t **swap;
...
void ch_table_compress(ch_table_t *t, unsigned flags)
{
    ch_table_entry_t **sort_array;
    ch_table_entry_t **sort_next;
    ch_table_entry_t **sort_end;
    ch_table_entry_t *table_next;
    ch_table_entry_t **last;
    int i;
    int dups_found;
     ...
     * duplicate keys
     */
    sort_array = (ch_table_entry_t **)
        ch_palloc(t->a.pool, t->a.nelts * sizeof(ch_table_entry_t*));
    sort_next = sort_array;
    table_next = (ch_table_entry_t *)t->a.elts;
    i = t->a.nelts;
    do {
    ...
    while (sort_next < sort_end) {
        if (((*sort_next)->key_checksum == (*last)->key_checksum) &&
            ...
            ch_table_entry_t **dup_last = sort_next + 1;
            dups_found = 1;
            ...
            if (flags == CH_OVERLAP_TABLES_MERGE) {
                ...
                new_val = (char *)ch_palloc(t->a.pool, len);
                val_dst = new_val;
                next = last;
                for (;;) {
    ...
    if (dups_found) {
        ch_table_entry_t *src = (ch_table_entry_t *)t->a.elts;
        ch_table_entry_t *dst = (ch_table_entry_t *)t->a.elts;
        ch_table_entry_t *last_elt = src + t->a.nelts;
        do {
            if (src->key) {
                *dst++ = *src;
            }
...
static void ch_table_cat(ch_table_t *t, const ch_table_t *s)
{
    const int n = t->a.nelts;
    register int idx;

    ch_array_cat(&t->a,&s->a);
...
void ch_table_overlap(ch_table_t *a, const ch_table_t *b,
				    unsigned flags)
{
...
static int table_getm_do(void *v, const char *key, const char *val)
{
    ...
    if (!state->first) {
        ...
        const char **elt;
        if (!state->merged) {
            state->merged = ch_array_make(state->p, 10, sizeof(const char *));
            elt = ch_array_push(state->merged);
            *elt = state->first;
        }
        elt = ch_array_push(state->merged);
        *elt = val;
...
const char * ch_table_getm(ch_pool_t *p, const ch_table_t *t,
        const char *key)
{
    table_getm_t state;
    ...
    ch_table_do(table_getm_do, &state, t, key, NULL);
    ...
    if (!state.first) {
        ...
        return ch_array_pstrcat(p, state.merged, ',');
    }
}
