 ...
 * <pre>
 *      struct my_element_t {
 *          CH_RING_ENTRY(my_element_t) link;
 *          int foo;
 *          char *bar;
 *      };
 * </pre>
 ...
 * than one CH_RING_ENTRY field. Each CH_RING_ENTRY has a corresponding
 * CH_RING_HEAD declaration.
 *
 * @warning For strict C standards compliance you should put the CH_RING_ENTRY
 ...
 * <pre>
 *      CH_RING_HEAD(my_ring_t, my_element_t);
 ...
 * to the start of each element, not to the CH_RING_ENTRY structure.
 *
 * <pre>
 *     +->+------+<-+  +->+------+<-+  +->+------+<-+
 ...
 * </pre>
 *
 * The CH_RING_HEAD is nothing but a bare CH_RING_ENTRY. The prev
 ...
 * name for its CH_RING_ENTRY in each element, and has its own type
 * for its CH_RING_HEAD.
 *
 * Note also that if the offset is non-zero (which is required if an
 * element has more than one CH_RING_ENTRY), the unreality of the
 * sentinel may have bad implications on very perverse implementations
 * of C -- see the warning in CH_RING_ENTRY.
 *
 * @param hp   The head of the ring
 * @param elem The name of the element struct
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_SENTINEL(hp, elem, link)				\
 ...
 * @param hp   The head of the ring
 */
#define CH_RING_FIRST(hp)	(hp)->next
/**
 * The last element of the ring
 * @param hp   The head of the ring
 */
#define CH_RING_LAST(hp)	(hp)->prev
/**
 * The next element in the ring
 * @param ep   The current element
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_NEXT(ep, link)	(ep)->link.next
/**
 * The previous element in the ring
 * @param ep   The current element
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_PREV(ep, link)	(ep)->link.prev
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_INIT(hp, elem, link) do {				\
	CH_RING_FIRST((hp)) = CH_RING_SENTINEL((hp), elem, link);	\
	CH_RING_LAST((hp))  = CH_RING_SENTINEL((hp), elem, link);	\
    } while (0)
 ...
 */
#define CH_RING_EMPTY(hp, elem, link)					\
    (CH_RING_FIRST((hp)) == CH_RING_SENTINEL((hp), elem, link))
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_ELEM_INIT(ep, link) do {				\
	CH_RING_NEXT((ep), link) = (ep);				\
	CH_RING_PREV((ep), link) = (ep);				\
    } while (0)
 ...
 *   empty rings... see CH_RING_SPLICE_HEAD for one that does
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_SPLICE_BEFORE(lep, ep1, epN, link) do {		\
	CH_RING_NEXT((epN), link) = (lep);				\
	CH_RING_PREV((ep1), link) = CH_RING_PREV((lep), link);	\
	CH_RING_NEXT(CH_RING_PREV((lep), link), link) = (ep1);	\
	CH_RING_PREV((lep), link) = (epN);				\
    } while (0)
 ...
 *   empty rings... see CH_RING_SPLICE_TAIL for one that does
 * @param lep  Element in the ring to splice after
 * @param ep1  First element in the sequence to splice in
 * @param epN  Last element in the sequence to splice in
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_SPLICE_AFTER(lep, ep1, epN, link) do {			\
	CH_RING_PREV((ep1), link) = (lep);				\
	CH_RING_NEXT((epN), link) = CH_RING_NEXT((lep), link);	\
	CH_RING_PREV(CH_RING_NEXT((lep), link), link) = (epN);	\
	CH_RING_NEXT((lep), link) = (ep1);				\
    } while (0)
 ...
 *   empty rings... see CH_RING_INSERT_HEAD for one that does
 * @param lep  Element in the ring to insert before
 * @param nep  Element to insert
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_INSERT_BEFORE(lep, nep, link)				\
	CH_RING_SPLICE_BEFORE((lep), (nep), (nep), link)
 ...
 *   empty rings... see CH_RING_INSERT_TAIL for one that does
 * @param lep  Element in the ring to insert after
 * @param nep  Element to insert
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_INSERT_AFTER(lep, nep, link)				\
	CH_RING_SPLICE_AFTER((lep), (nep), (nep), link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_SPLICE_HEAD(hp, ep1, epN, elem, link)			\
	CH_RING_SPLICE_AFTER(CH_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_SPLICE_TAIL(hp, ep1, epN, elem, link)			\
	CH_RING_SPLICE_BEFORE(CH_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_INSERT_HEAD(hp, nep, elem, link)			\
	CH_RING_SPLICE_HEAD((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_INSERT_TAIL(hp, nep, elem, link)			\
	CH_RING_SPLICE_TAIL((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_CONCAT(h1, h2, elem, link) do {			\
	if (!CH_RING_EMPTY((h2), elem, link)) {			\
	    CH_RING_SPLICE_BEFORE(CH_RING_SENTINEL((h1), elem, link),	\
				  CH_RING_FIRST((h2)),			\
				  CH_RING_LAST((h2)), link);		\
	    CH_RING_INIT((h2), elem, link);				\
	}								\
    } while (0)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_PREPEND(h1, h2, elem, link) do {			\
	if (!CH_RING_EMPTY((h2), elem, link)) {			\
	    CH_RING_SPLICE_AFTER(CH_RING_SENTINEL((h1), elem, link),	\
				  CH_RING_FIRST((h2)),			\
				  CH_RING_LAST((h2)), link);		\
	    CH_RING_INIT((h2), elem, link);				\
	}								\
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_UNSPLICE(ep1, epN, link) do {				\
	CH_RING_NEXT(CH_RING_PREV((ep1), link), link) =		\
		     CH_RING_NEXT((epN), link);			\
	CH_RING_PREV(CH_RING_NEXT((epN), link), link) =		\
		     CH_RING_PREV((ep1), link);			\
    } while (0)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_REMOVE(ep, link)					\
    CH_RING_UNSPLICE((ep), (ep), link)
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_FOREACH(ep, head, elem, link)                          \
    for (ep = CH_RING_FIRST(head);                                     \
         ep != CH_RING_SENTINEL(head, elem, link);                     \
         ep = CH_RING_NEXT(ep, link))
 ...
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_FOREACH_SAFE(ep1, ep2, head, elem, link)               \
    for (ep1 = CH_RING_FIRST(head), ep2 = CH_RING_NEXT(ep1, link);    \
         ep1 != CH_RING_SENTINEL(head, elem, link);                    \
         ep1 = ep2, ep2 = CH_RING_NEXT(ep1, link))
...
#define CH_RING_CHECK_ONE(msg, ptr)					\
	fprintf(stderr, "*** %s %p\n", msg, ptr)

#define CH_RING_CHECK(hp, elem, link, msg)				\
	CH_RING_CHECK_ELEM(CH_RING_SENTINEL(hp, elem, link), elem, link, msg)

#define CH_RING_CHECK_ELEM(ep, elem, link, msg) do {			\
	...
	do {								\
	    ...
	    fprintf(stderr, "\telem->next %p\n",			\
		    CH_RING_NEXT(here, link));				\
	    fprintf(stderr, "\telem->prev %p\n",			\
		    CH_RING_PREV(here, link));				\
	    fprintf(stderr, "\telem->next->prev %p\n",			\
		    CH_RING_PREV(CH_RING_NEXT(here, link), link));	\
	    fprintf(stderr, "\telem->prev->next %p\n",			\
		    CH_RING_NEXT(CH_RING_PREV(here, link), link));	\
	    if (CH_RING_PREV(CH_RING_NEXT(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->next->prev != elem\n");	\
		break;							\
	    }								\
	    if (CH_RING_NEXT(CH_RING_PREV(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->prev->next != elem\n");	\
		break;							\
	    }								\
	    here = CH_RING_NEXT(here, link);				\
	} while (here != start);					\
	fprintf(stderr, "*** ring check end\n");			\
    } while (0)

#define CH_RING_CHECK_CONSISTENCY(hp, elem, link)			\
	CH_RING_CHECK_ELEM_CONSISTENCY(CH_RING_SENTINEL(hp, elem, link),\
					elem, link)

#define CH_RING_CHECK_ELEM_CONSISTENCY(ep, elem, link) do {		\
	struct elem *start = (ep);					\
	struct elem *here = start;					\
	do {								\
	    assert(CH_RING_PREV(CH_RING_NEXT(here, link), link) == here); \
	    assert(CH_RING_NEXT(CH_RING_PREV(here, link), link) == here); \
	    here = CH_RING_NEXT(here, link);				\
	} while (here != start);					\
    } while (0)
 ...
 *   (This is a no-op unless CH_RING_DEBUG is defined.)
 ...
 * an assertion if any inconsistency is found.  Same idea as CH_RING_CHECK()
 ...
 * @param msg  Descriptive message
 */
#define CH_RING_CHECK_ELEM(ep, elem, link, msg)
/**
 ...
 * is found.  Same idea as CH_RING_CHECK_ELEM() except that it's silent
 * if all is well.
 *   (This is a no-op unless CH_RING_DEBUG is defined.)
 * @param ep   The element
 * @param elem The name of the element struct
 * @param link The name of the CH_RING_ENTRY in the element struct
 */
#define CH_RING_CHECK_ELEM_CONSISTENCY(ep, elem, link)
#endif
