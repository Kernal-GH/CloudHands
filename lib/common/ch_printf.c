...
static char *ch_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    double fi, fj;
...
static char *ch_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (ch_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *ch_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (ch_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * ch_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *ch_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = ch_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
    ...
    char *buf, size_t *len)
{
    ...
    if (format == 'f')
        p = ch_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = ch_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
 ...
 */
int ch_vformatter(int (*flush_func)(ch_vformatter_buff_t *),
    ch_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        if (*fmt != '%') {
            ...
            /*
             * Modifier check.  In same cases, CH_OFF_T_FMT can be
             * "lld" and CH_INT64_T_FMT can be "ld" (that is, off_t is
             * "larger" than int64). Check that case 1st.
             * Note that if CH_OFF_T_FMT is "d",
             * the first if condition is never true. If CH_INT64_T_FMT
             * is "d' then the second if condition is never true.
             */
            if ((sizeof(CH_OFF_T_FMT) > sizeof(CH_INT64_T_FMT)) &&
                ((sizeof(CH_OFF_T_FMT) == 4 &&
                 fmt[0] == CH_OFF_T_FMT[0] &&
                 fmt[1] == CH_OFF_T_FMT[1]) ||
                (sizeof(CH_OFF_T_FMT) == 3 &&
                 fmt[0] == CH_OFF_T_FMT[0]) ||
                (sizeof(CH_OFF_T_FMT) > 4 &&
                 strncmp(fmt, CH_OFF_T_FMT, 
                         sizeof(CH_OFF_T_FMT) - 2) == 0))) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(CH_OFF_T_FMT) - 2);
            }
            else if ((sizeof(CH_INT64_T_FMT) == 4 &&
                 fmt[0] == CH_INT64_T_FMT[0] &&
                 fmt[1] == CH_INT64_T_FMT[1]) ||
                (sizeof(CH_INT64_T_FMT) == 3 &&
                 fmt[0] == CH_INT64_T_FMT[0]) ||
                (sizeof(CH_INT64_T_FMT) > 4 &&
                 strncmp(fmt, CH_INT64_T_FMT, 
                         sizeof(CH_INT64_T_FMT) - 2) == 0)) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(CH_INT64_T_FMT) - 2);
            }
            else if (*fmt == 'q') {
                var_type = IS_QUAD;
                fmt++;
            ...
            case 'G':
                 ...
                 * * We use &num_buf[ 1 ], so that we have room for the sign
                 */
                s = ch_gcvt(va_arg(ap, double), (int) precision, &num_buf[1],
                            alternate_form);
                if (*s == '-')
                    prefix_char = *s++;
                else if (print_sign)
...
static int snprintf_flush(ch_vformatter_buff_t *vbuff)
{
    /* if the buffer fills we have to abort immediately, there is no way
     * to "flush" an ch_snprintf... there's nowhere to flush it to.
     */
	vbuff = vbuff;
...
int ch_snprintf(char *buf, size_t len, 
                                     const char *format, ...)
{
    int cc;
    va_list ap;
    ...
    va_start(ap, format);
    cc = ch_vformatter(snprintf_flush, &vbuff, format, ap);
    va_end(ap);
    if (len != 0) {
        *vbuff.curpos = '\0';
    }
...
int ch_vsnprintf(char *buf, size_t len, const char *format,
                               va_list ap)
{
    int cc;
    ch_vformatter_buff_t vbuff;
...
static int psprintf_flush(ch_vformatter_buff_t *vbuff)
{
...
char * ch_pvsprintf(ch_pool_t *pool, const char *fmt, va_list ap)
{
    ...
    if (ch_vformatter(psprintf_flush, &ps.vbuff, fmt, ap) == -1) {
