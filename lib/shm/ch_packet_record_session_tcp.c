 ...
 * Last Modified: 2018-06-06 18:28:47
 */
...
void ch_packet_record_session_tcp_read(ch_packet_record_session_tcp_t *pkt_tcp_session,ch_packet_record_t *pkt_rcd){
	...
	pkt_tcp_session->req_data = (void*)req_data;
	pkt_tcp_session->req_dsize = req_dsize;

	pkt_tcp_session->res_data = (void*)res_data;
	pkt_tcp_session->res_dsize = res_dsize;
...
size_t ch_packet_record_session_tcp_write(ch_packet_record_session_tcp_t *pkt_tcp_session,void *buffer,size_t bsize,void *data,size_t dlen){
	...
	ch_bf_bytes_write(bfmt,(pkt_tcp_session->req_data == NULL||pkt_tcp_session->req_dsize==0)?"":pkt_tcp_session->req_data,
		pkt_tcp_session->req_dsize);

	ch_bf_bytes_write(bfmt,(pkt_tcp_session->res_data == NULL||pkt_tcp_session->res_dsize==0)?"":pkt_tcp_session->res_data,
		pkt_tcp_session->res_dsize);
