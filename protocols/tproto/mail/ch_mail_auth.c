 ...
 * Last Modified: 2016-08-02 17:27:29
 */
					...
					const char *buf, uint32_t len)
{
	...
	plain.data = (unsigned char *)apr_pcalloc(pool, len);
	if (plain.data == NULL) {
		return -1;
	}
...
int ch_mail_auth_login_username(apr_pool_t *pool, ch_str_t *user, const uint8_t *buf, uint32_t len)
{
	...
	user->data = (unsigned char *)apr_pcalloc(pool, len);
	if (user->data == NULL)
		return -1;
...
int ch_mail_auth_login_password(apr_pool_t *pool, ch_str_t *pass, const uint8_t *buf, uint32_t len)
{
	...
	pass->data = (unsigned char *)apr_pcalloc(pool, len);
	if (pass->data == NULL)
		return -1;
