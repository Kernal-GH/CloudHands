 ...
 * Last Modified: 2016-11-03 03:50:39
 */
...
struct mail_header_operator_t {
	char *name;
	size_t name_len;
	int (*oper_act)(const char *value, uint32_t vlen, apr_pool_t *pool, ch_mail_session_t *s);
};

static int 
_process_subject(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s);

static int 
_process_mail_from(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s);

static int 
_process_mail_to(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s);

static int 
_process_mail_cc(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s);
...
_process_subject(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s)
{
	s->subject.data = (unsigned char *)apr_pstrndup(pool, value, vlen);
	if (s->subject.data == NULL) {
		CH_ERR_LOG("%s, apr_pstrndup failed.\n", __func__);
		return -1;
	}
...
_process_mail_from(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s)
{
	s->mail_from.data = (unsigned char *)apr_pstrndup(pool, value, vlen);
...
static int 
__process_mail_to(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s)
{
	mail_addr_node_t *mail_addr;

	mail_addr = (mail_addr_node_t *) apr_palloc(pool, sizeof(mail_addr_node_t));
	if (mail_addr == NULL) {
		CH_ERR_LOG("%s, apr_palloc failed.\n", __func__);
		return -1;
	}
	mail_addr->name.data = (unsigned char *)apr_pstrndup(pool, value, vlen);
...
static int 
__process_mail_cc(const char *value, uint32_t vlen, apr_pool_t *pool,
	ch_mail_session_t *s)
{
	mail_addr_node_t *mail_addr;
...
mail_process_header(const uint8_t *name, uint32_t nlen, const uint8_t *value,
	uint32_t vlen, apr_pool_t *pool, ch_mail_session_t *s)
{
...
_get_file_node_for_mail_ctnt_save(apr_pool_t *pool, 
	ch_mail_proto_t *mpproto, int is_attach)
{
	ch_mail_ctnt_t *ctnt;
	char *fname, *fname_tmp;
	ch_fpath_t *fpath = is_attach ? mpproto->attach_fpath : mpproto->content_fpath;
...
static int 
mail_process_content_plain_text(const uint8_t *chunk, uint32_t len,
	MimeDecParseState *state, ch_mail_session_t *s)
{
	...
	if (state->body_begin) {
		...
		ctnt = _get_file_node_for_mail_ctnt_save(sentry->mp, sentry->mpproto, 0);
...
static int 
mail_process_content_html(const uint8_t *chunk, uint32_t len,
	MimeDecParseState *state, ch_mail_session_t *s)
{
	ch_mail_ctnt_t *ctnt = NULL;
	ch_mail_session_entry_t *sentry = NULL;
...
static int 
mail_process_attachment(const uint8_t *chunk, uint32_t len, 
	MimeDecParseState *state, ch_mail_session_t *s)
{
	ch_mail_ctnt_t *ctnt = NULL;
	ch_mail_session_entry_t *sentry = NULL;
	...
	if (state->body_begin) {
		/* get file path and append data */
		ctnt = _get_file_node_for_mail_ctnt_save(sentry->mp, sentry->mpproto, 1);
								
		if (ctnt == NULL) {
			CH_ERR_LOG("%s: add file_node_s failed.\n", __func__);
			return MIME_DEC_ERR_DATA;
		}
		ctnt->filename = apr_pstrndup(sentry->mp, (const char *)entity->filename, 
			entity->filename_len);
...
MailProcessDataChunk(const uint8_t *chunk, uint32_t len, MimeDecParseState *state)
{
	...
	if (entity->ctnt_flags & CTNT_IS_ATTACHMENT) {
		ret = mail_process_attachment(chunk, len, state, s);
	} else if (entity->ctnt_flags & CTNT_IS_TEXT) {
		ret = mail_process_content_plain_text(chunk, len, state, s);
	} else if (entity->ctnt_flags & CTNT_IS_HTML) {
		ret = mail_process_content_html(chunk, len, state, s);
	}
