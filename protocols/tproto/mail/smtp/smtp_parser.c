 ...
 * Last Modified: 2016-10-28 02:31:25
 *************************************************************************/
...
static void *ch_mail_smtp_state_create(apr_pool_t *pool)
{
	...
	smtp_state = (smtp_state_t *) apr_pcalloc(pool, sizeof(smtp_state_t));
	if (smtp_state == NULL)
		return NULL;

	smtp_state->cmds = (uint8_t *) apr_pcalloc(pool, 
						sizeof(uint8_t) * SMTP_COMMAND_BUFFER_STEPS);
	if (smtp_state->cmds == NULL) {
		return NULL;
	}
	smtp_state->cmds_buffer_len = SMTP_COMMAND_BUFFER_STEPS;
	CH_INIT_LIST_HEAD(&smtp_state->rcpt_list);

	return smtp_state;
}

static int
smtp_insert_cmd_buffer(uint8_t command, ch_mail_session_entry_t *sentry)
{
	void *ptmp;
	apr_pool_t *pool = sentry->mp;
	smtp_state_t *state = (smtp_state_t *) sentry->priv_data;
...
static int 
smtp_process_cmd_starttls(ch_mail_session_entry_t *sentry, 
...
static int 
smtp_process_cmd_data(ch_mail_session_entry_t *sentry, 
	ch_mail_curr_line_t *cur_line)
{
	ch_mail_session_t *s = sentry->curr_session;

    if (!(sentry->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE)) {
        /* looks like are still waiting for a confirmination from the server */
        return 0;
    }

    if (cur_line->len == 1 && cur_line->data[0] == '.') {
        sentry->parser_state &= ~SMTP_PARSER_STATE_COMMAND_DATA_MODE;
         ...
         * the reply received */
        smtp_insert_cmd_buffer(SMTP_COMMAND_DATA_MODE, sentry);
    ...
    if (sentry->current_command == SMTP_COMMAND_DATA &&
            (sentry->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE)) {
...
static int 
smtp_process_cmd_bdat(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	smtp_state_t *state = (smtp_state_t *) sentry->priv_data;
...
static int
smtp_parse_cmd_with_param(ch_mail_session_entry_t *sentry,
					ch_mail_curr_line_t *cur_line,
					uint8_t prefix_len,
					uint8_t **target,
					uint16_t *target_len)
{
	...
	*target = (uint8_t *) apr_pstrndup(sentry->mp, (const char *)cur_line->data + i, spc_i - i);
	if (*target == NULL)
		return -1;
...
static int 
smtp_parse_command_bdat(ch_mail_session_entry_t *sentry, 
	ch_mail_curr_line_t *cur_line)
{
    int i = 4;
	smtp_state_t *state = (smtp_state_t *) sentry->priv_data;
...
static int 
smtp_parse_command_helo(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	smtp_state_t *state = (smtp_state_t *) sentry->priv_data;

	return smtp_parse_cmd_with_param(sentry, cur_line, 4, 
		&state->helo, &state->helo_len);
}

static int 
smtp_parse_command_mailfrom(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	smtp_state_t *state = (smtp_state_t *) sentry->priv_data;

	return smtp_parse_cmd_with_param(sentry, cur_line, 9,
		(uint8_t **)&state->smtp_from.data,
		(uint16_t *)&state->smtp_from.len);
}

static int 
smtp_parse_command_auth_plain(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	...
	return ch_mail_auth_plain(pool, &sentry->login, 
								&sentry->passwd,
								ptr, end - ptr);
}

static int 
smtp_parse_command_auth_login_username(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	int ret;

	ret = ch_mail_auth_login_username(sentry->mp, &sentry->login,
		cur_line->data, cur_line->len);

	sentry->parser_state &= ~SMTP_PARSER_STATE_AUTH_USERNAME;

	sentry->parser_state |= SMTP_PARSER_STATE_AUTH_PASSWORD;

	return ret;
}

static int 
smtp_parse_command_auth_login_password(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	int ret;

	ret = ch_mail_auth_login_password(sentry->mp, &sentry->passwd,
		cur_line->data, cur_line->len);
...
static int 
smtp_parse_command_auth_login(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	/* unused */
	sentry = sentry;
...
static int 
smtp_parse_command_auth_cram_md5(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	/* unused */
	sentry = sentry;
...
static int 
smtp_parse_command_auth(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	if (cur_line->len >= 10 && \
		strncasecmp("auth plain", (char *)cur_line->data, 10) == 0) {
		return smtp_parse_command_auth_plain(sentry, cur_line);
	} else if (cur_line->len >= 10 && \
		strncasecmp("auth login", (char *)cur_line->data, 10) == 0) {
		return smtp_parse_command_auth_login(sentry, cur_line);
	} else if (cur_line->len >= 13 && \
		strncasecmp("auth cram-md5", (char *)cur_line->data, 13) == 0) {
		return smtp_parse_command_auth_cram_md5(sentry, cur_line);
	}
	return -1;
}

static int 
smtp_parse_command_rcptto(ch_mail_session_entry_t *sentry,
	ch_mail_curr_line_t *cur_line)
{
	uint8_t *rcptto;
	uint16_t rcptto_len;
	...
	if (smtp_parse_cmd_with_param(sentry, cur_line, 7, &rcptto, &rcptto_len) == 0) {
		mail_addr_node_t *rcptto_node = \
			(mail_addr_node_t *)apr_palloc(pool, sizeof(mail_addr_node_t));
		if (rcptto_node) {
			...
			CH_ERR_LOG("%s, call apr_palloc failed.\n");
			return -1;
		}
	} else {
		return -1;
...
static int 
smtp_process_request(ch_mail_session_entry_t *sentry, ch_mail_curr_line_t *cur_line)
{
	ch_mail_session_t *s = sentry->curr_session;
	apr_pool_t *pool = sentry->mp;
    ...
    if (!(sentry->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE)) {
        ...
        if (cur_line->len >= 8 &&
            ...
            sentry->current_command = SMTP_COMMAND_STARTTLS;
		} else if (cur_line->len >= 4 &&
			strncasecmp("data", (char *)cur_line->data, 4) == 0) {
			sentry->current_command = SMTP_COMMAND_DATA;
            ...
            r = smtp_parse_command_bdat(sentry, cur_line);
            if (r == -1) {
                return -1;
            }
            sentry->current_command = SMTP_COMMAND_BDAT;
            sentry->parser_state |= SMTP_PARSER_STATE_COMMAND_DATA_MODE;
        } else if (cur_line->len >= 4 &&
                   ((strncasecmp("helo", (char *)cur_line->data, 4) == 0) ||
                    strncasecmp("ehlo", (char *)cur_line->data, 4) == 0))  {
            r = smtp_parse_command_helo(sentry, cur_line);
            if (r == -1) {
                return -1;
            }
            sentry->current_command = SMTP_COMMAND_OTHER_CMD;
        } else if (cur_line->len >= 4 &&
                   strncasecmp("auth", (char *)cur_line->data, 4) == 0) {
			r = smtp_parse_command_auth(sentry, cur_line);
			if (r == -1) {
				return -1;
			}
			sentry->current_command = SMTP_COMMAND_AUTH;
		} else if (cur_line->len >= 9 &&
                   strncasecmp("mail from", (char *)cur_line->data, 9) == 0) {
            r = smtp_parse_command_mailfrom(sentry, cur_line);
            if (r == -1) {
                return -1;
            }
            sentry->current_command = SMTP_COMMAND_OTHER_CMD;
            ...
            r = smtp_parse_command_rcptto(sentry, cur_line);
            if (r == -1) {
                return -1;
            }
            sentry->current_command = SMTP_COMMAND_OTHER_CMD;
                   ...
                   strncasecmp("quit", (char *)cur_line->data, 4) == 0) {
            sentry->current_command = SMTP_COMMAND_QUIT;
		}else {
			if (sentry->parser_state & SMTP_PARSER_STATE_AUTH_USERNAME) {
				smtp_parse_command_auth_login_username(sentry, cur_line);
			} else if (sentry->parser_state & SMTP_PARSER_STATE_AUTH_PASSWORD) {
				smtp_parse_command_auth_login_password(sentry, cur_line);
			} else {
				sentry->current_command = SMTP_COMMAND_OTHER_CMD;
			}
        }
        ...
        if (smtp_insert_cmd_buffer(sentry->current_command, sentry) == -1) {
            return -1;
        }
	...
	case SMTP_COMMAND_STARTTLS:
		return smtp_process_cmd_starttls(sentry, cur_line);

	case SMTP_COMMAND_DATA:
		return smtp_process_cmd_data(sentry, cur_line);

	case SMTP_COMMAND_BDAT:
		return smtp_process_cmd_bdat(sentry, cur_line);
...
static int 
smtp_process_reply(ch_mail_session_entry_t *sentry, ch_mail_curr_line_t *cur_line)
{
    ...
    if (cur_line->len >= 4) {
        if (sentry->parser_state & SMTP_PARSER_STATE_PARSING_MULTILINE_REPLY) {
            if (cur_line->data[3] != '-') {
                sentry->parser_state &= ~SMTP_PARSER_STATE_PARSING_MULTILINE_REPLY;
            }
        } else {
            if (cur_line->data[3] == '-') {
                sentry->parser_state |= SMTP_PARSER_STATE_PARSING_MULTILINE_REPLY;
    ...
    if (state->cmds_idx == state->cmds_cnt) {
        if (!(sentry->parser_state & SMTP_PARSER_STATE_FIRST_REPLY_SEEN)) {
             ...
             */
            if (!(sentry->parser_state & SMTP_PARSER_STATE_PARSING_MULTILINE_REPLY)) {
                sentry->parser_state |= SMTP_PARSER_STATE_FIRST_REPLY_SEEN;
				//fprintf(stderr, "id: %d, set first reply seen flag.\n", sentry->tbl_entry.session_id);
			} 
    ...
    if (state->cmds_cnt == 0) {
	    ...
	    if ((reply_code == SMTP_REPLY_334) && \
			    ((sentry->parser_state & SMTP_PARSER_STATE_AUTH_USERNAME) == 0) && \
			    ((sentry->parser_state & SMTP_PARSER_STATE_AUTH_PASSWORD) == 0))
	    {
		    sentry->parser_state |= SMTP_PARSER_STATE_AUTH_USERNAME;
	    }
	}
...
static int smtp_parse(int direction, ch_mail_proto_t *mpproto, 
	ch_mail_session_entry_t *sentry, uint8_t *input, uint32_t input_len)
{
    ...
    if (direction == 0) {
		...
		{
            if (smtp_process_request(sentry, &cur_line) == -1)
				return PARSE_BREAK;
		}
		...
		{
			if (smtp_process_reply(sentry, &cur_line) == -1)
				return PARSE_BREAK;
		}
	}
	...
	uint32_t dlen, void *priv_data)
{
	...
	if (sentry->priv_data == NULL) {
		sentry->priv_data = ch_mail_smtp_state_create(sentry->mp);
	...
	return smtp_parse(0, mpproto, sentry, data, dlen);
}
	...
	uint32_t dlen, void *priv_data)
{
	...
	return smtp_parse(1, mpproto, sentry, data, dlen);
}
