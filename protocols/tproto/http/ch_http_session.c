 ...
 * Last Modified: 2016-07-31 19:21:50
 */
...
static inline void _http_headers_out_init(ch_http_headers_out_t *hout){
	...
	CH_INIT_LIST_HEAD(&hout->headers);
	hout->server = NULL;
...
ch_http_session_t * ch_http_session_create(apr_pool_t *mp){

	ch_http_session_t *session = (ch_http_session_t*)apr_palloc(mp,sizeof(ch_http_session_t));
...
static inline unsigned int _http_version(unsigned int major,unsigned int minor){

	 return major * 1000 + minor;
}

static inline void _strndup(apr_pool_t *mp,unsigned char **p,unsigned char *start,unsigned char *end,int incr){

	if(start == NULL || end == NULL||start==end)
		*p = (unsigned char*)"";
	else
		*p = (unsigned char*)apr_pstrndup(mp,(const char*)start,(size_t)((end-start)+incr));
...
void ch_http_session_request_line_set(ch_http_session_t *session,ch_http_request_line_t *req_line){
	...
	/*set method*/
	_strndup(session->mp,&session->method,req_line->request_start,req_line->method_end,1);

	/* set http version */
	session->http_version = _http_version(req_line->http_major,req_line->http_minor);

	/*set uri */
	_strndup(session->mp,&session->uri,req_line->uri_start,req_line->uri_end,0);

}

static inline size_t _strtosize(unsigned char *v){
	...
	return (size_t)strtoul((const char*)v,&end,10);
}
    ...
    switch(len){
        ...
        case 10:
            ...
            if(strcasecmp((const char*)header->key,"Connection") == 0){
				...
				if (ch_strcasestrn(header->val, "close", 5 - 1)) {
					session->headers_in.connection_type = CH_HTTP_CONNECTION_CLOSE;

				} else if (ch_strcasestrn(header->val, "keep-alive", 10 - 1)) {
        ...
        case 14:
            ...
            if(strcasecmp((const char*)header->key,"Content-Length") == 0){
				...
				session->headers_in.content_length_n = _strtosize(header->val);
            }
    ...
    switch(len){
        ...
        case 14:
            ...
            if(strcasecmp((const char*)header->key,"Content-Length") == 0){
				...
				session->headers_out.content_length_n = _strtosize(header->val);
            }
...
void ch_http_session_header_add(ch_http_session_t *session,ch_http_header_t *header,int is_req){
	...
	ch_table_elt_t *entry = (ch_table_elt_t*)apr_palloc(session->mp,sizeof(ch_table_elt_t));

	_strndup(session->mp,&entry->key,header->header_name_start,header->header_name_end,0);
	_strndup(session->mp,&entry->val,header->header_start,header->header_end,0);
...
void ch_http_session_body_append(ch_http_session_t *session,void *data,size_t dlen,int is_req){
