...
int ch_http_request_line_parse(ch_http_request_line_t *line)
{
	unsigned char  c, ch, *p, *m;
	ch_buf_t *b = line->pdata.pb;
    ...
    state = line->pdata.state;

    for (p = (unsigned char*)b->pos; p < (unsigned char*)b->end; p++) {
        ch = *p;
        ...
        case sw_method:
            if (ch == ' ') {
                line->method_end = p - 1;
                m = line->request_start;

                switch (p - m) {

                case 3:
                    if (ch_str3_cmp(m, 'G', 'E', 'T', ' ')) {
                        line->method = CH_HTTP_GET;
                        break;
                    }

                    if (ch_str3_cmp(m, 'P', 'U', 'T', ' ')) {
                        line->method = CH_HTTP_PUT;
                        break;
                    }
                ...
                case 4:
                    if (m[1] == 'O') {
                        ...
                        if (ch_str3Ocmp(m, 'P', 'O', 'S', 'T')) {
                            line->method = CH_HTTP_POST;
                            break;
                        }

                        if (ch_str3Ocmp(m, 'C', 'O', 'P', 'Y')) {
                            line->method = CH_HTTP_COPY;
                            break;
                        }

                        if (ch_str3Ocmp(m, 'M', 'O', 'V', 'E')) {
                            line->method = CH_HTTP_MOVE;
                            break;
                        }

                        if (ch_str3Ocmp(m, 'L', 'O', 'C', 'K')) {
                            line->method = CH_HTTP_LOCK;
                            break;
                        }
                ...
                case 5:
                    if (ch_str5cmp(m, 'M', 'K', 'C', 'O', 'L')) {
                        line->method = CH_HTTP_MKCOL;
                        break;
                    }

                    if (ch_str5cmp(m, 'P', 'A', 'T', 'C', 'H')) {
                        line->method = CH_HTTP_PATCH;
                        break;
                    }

                    if (ch_str5cmp(m, 'T', 'R', 'A', 'C', 'E')) {
                        line->method = CH_HTTP_TRACE;
                        break;
                    }
                ...
                case 6:
                    if (ch_str6cmp(m, 'D', 'E', 'L', 'E', 'T', 'E')) {
                        line->method = CH_HTTP_DELETE;
                        break;
                    }

                    if (ch_str6cmp(m, 'U', 'N', 'L', 'O', 'C', 'K')) {
                        line->method = CH_HTTP_UNLOCK;
                        break;
                    }
    ...
    line->http_version = line->http_major * 1000 + line->http_minor;
    line->pdata.state = sw_start;

    if (line->http_version == 9 && line->method != CH_HTTP_GET) {
...
int ch_http_header_line_parse(ch_http_header_t *header)
{
	...
	ch_buf_t *b = header->pdata.pb;



    state = header->pdata.state;
...
int ch_http_status_line_parse(ch_http_status_line_t *status)
{
	...
	ch_buf_t *b = status->pdata.pb;

    state = status->pdata.state;
...
int ch_http_chunked_parse(ch_http_chunked_t *ctx){
    ...
    state = ctx->pdata.state;
	ch_buf_t *b = ctx->pdata.pb;
    ...
    for (pos = (unsigned char*)b->pos; pos < (unsigned char*)b->end; pos++) {
