 ...
 * Last Modified: 2017-12-28 18:30:34
 */
...
ch_ptable_t * ch_ptable_create(apr_pool_t *mp,int pool_type,
        size_t entry_size,size_t n_entries_limit,size_t tbl_size,
	...
	tbl->ep = ch_entry_pool_create(mp,pool_type,entry_size,n_entries_limit,n_entries_limit);
	if(tbl->ep == NULL){
    ...
    tbl = (ch_ptable_t*)apr_palloc(mp,msize);
    if(tbl == NULL){
		ch_entry_pool_destroy(tbl->ep);
...
static inline void tbl_entry_free(ch_ptable_t *tbl,ch_ptable_entry_t *tbl_entry,int is_timeout){
...
static inline int _entry_is_timeout(ch_ptable_t *tbl,ch_ptable_entry_t *tbl_entry){
