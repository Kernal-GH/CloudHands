...
# Last Modified: 2017-10-09 17:52:55
#
...
echo "------------------------------------------------------------------------------"
. $RTE_SDK/tools/setup_config

HUGEPGSZ=`cat /proc/meminfo  | grep Hugepagesize | cut -d : -f 2 | tr -d ' '`
ASS_NAME=CloudHands/cloudhands
ASS_CONF=CloudHands/conf/cloudhands.conf
...
#
#setup cloudhands to capture packets,and reasseble flow
#
setup_cloudhands(){

	${RTE_SDK}/${ASS_NAME} -n${N_CORE} -c${MASK} ${RTE_SDK}/${ASS_CONF} 1>/dev/null 2>&1 &
}
...
start_all(){
	...
	echo "setup assemble application based dpdk----------------------"
	setup_cloudhands
}
...
stop_all(){
	...
	echo "stop assemble application -----------------"
	pkill -9 cloudhands
