...
static inline void * ch_mmap_priv_data_address(ch_mmap_t *mm){

	ch_mmap_header_t *mh = mm->mmap_header;

	if(mh->mmap_priv_data_size == 0)
		return NULL;

	return (void*)(mh+1);
}

static inline uint64_t ch_mmap_entry_pos_offset(ch_mmap_t *mm,uint64_t indx){

    return mm->mmap_header->mmap_entry_size*indx+mm->mmap_header->mmap_entries_start;
}
...
extern ch_mmap_t * ch_mmap_create(apr_pool_t *mp,const char *fname,uint64_t fsize,uint64_t  entry_size,uint64_t priv_data_size,int is_write);
