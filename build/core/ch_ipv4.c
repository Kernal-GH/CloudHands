...
static int ipv4_rcv(ch_packet_type_t *ptype,struct rte_mbuf *mbuf){

    ch_protocol_type_t *proto;

    struct ipv4_hdr *ip_hdr = ch_ipv4_hdr_get(mbuf);
    ...
    return proto->handler(proto,mbuf); 
}
