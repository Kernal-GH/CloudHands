...
static int 
smtp_process_cmd_data(ch_mail_session_entry_t *msentry, 
	ch_mail_curr_line_t *cur_line)
{
...
static int 
smtp_process_request(ch_mail_session_entry_t *msentry, ch_mail_curr_line_t *cur_line)
{
	ch_mail_session_t *s = msentry->curr_session;
	ch_pool_t *pool = msentry->mp;
	...
	case SMTP_COMMAND_DATA:
		return smtp_process_cmd_data(msentry, cur_line);
...
static int 
smtp_process_reply(ch_mail_session_entry_t *msentry, ch_mail_curr_line_t *cur_line)
{
...
static int smtp_parse(int direction,ch_mail_session_entry_t *msentry, uint8_t *input, uint32_t input_len)
{
	ch_mail_curr_line_t cur_line;
	ch_str_t input_data;
    ...
    if (direction == 0) {
		...
		{
            if (smtp_process_request(msentry, &cur_line) == -1)
				return PARSE_BREAK;
		}
		...
		{
			if (smtp_process_reply(msentry, &cur_line) == -1)
				return PARSE_BREAK;
		}
	}
...
int mail_smtp_request_parse(ch_mail_session_entry_t *msentry, void *data, size_t dlen)
{
	...
	return smtp_parse(0, msentry, data, dlen);
}

int mail_smtp_response_parse(ch_mail_session_entry_t *msentry, void *data, size_t dlen)
{
	if (msentry->priv_data == NULL) {
		msentry->priv_data = ch_mail_smtp_state_create(msentry->mp);
	...
	return smtp_parse(1, msentry, data, dlen);
}
