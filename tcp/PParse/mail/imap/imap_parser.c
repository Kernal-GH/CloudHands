...
static int 
imap_process_request(ch_mail_session_entry_t *msentry, ch_mail_curr_line_t *cur_line)
{
	ch_mail_session_t *s = msentry->curr_session;
	ch_pool_t *pool = msentry->mp;
	int r = 0;
...
static int imap_set_mime_decode_complete(ch_mail_session_entry_t *msentry)
{
	ch_mail_session_t *s = msentry->curr_session;
	int ret = -1;
...
static int 
imap_process_uid_fetch_data(ch_mail_session_entry_t *msentry, 
	ch_mail_curr_line_t *cur_line)
{
	imap_state_t *state = (imap_state_t *) msentry->priv_data;
	...
	if (cur_line->len == 1 && cur_line->data[0] == ')') {
		msentry->parser_state &= ~IMAP_STATE_DATA_UID_FETCH;
		imap_set_mime_decode_complete(msentry);
		msentry->current_command = IMAP_CMD_OTHER_CMD;
...
static int 
imap_process_reply(ch_mail_session_entry_t *msentry,
	ch_mail_curr_line_t *cur_line)
{
	...
	if (!(msentry->parser_state & IMAP_STATE_MULTILINE_REPLY)) {
		...
		if (msentry->parser_state & IMAP_STATE_DATA_UID_FETCH) {
			return imap_process_uid_fetch_data(msentry, cur_line);
		}
	}
...
static int imap_parse(int dir, ch_mail_session_entry_t *msentry, uint8_t *input, uint32_t input_len)
{
	ch_mail_curr_line_t cur_line;
	ch_str_t input_data;
    ...
    if (dir == 0) {
		...
		{
            if (imap_process_request(msentry, &cur_line) == -1)
				return PARSE_BREAK;
        }
		...
		{
            if (imap_process_reply(msentry, &cur_line) == -1)
				return PARSE_BREAK;
        }
    }
...
int mail_imap_request_parse(ch_mail_session_entry_t *msentry, void *data,size_t dlen)
{
	...
	return imap_parse(0, msentry, data, dlen);
}

int mail_imap_response_parse(ch_mail_session_entry_t *msentry, void *data,size_t dlen)
{
	if (msentry->priv_data == NULL) {
		msentry->priv_data = ch_mail_imap_state_create(msentry->mp);
	...
	return imap_parse(1, msentry, data, dlen);
}
