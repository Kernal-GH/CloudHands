...
_get_file_node_for_mail_ctnt_save(ch_pool_t *pool, 
	ch_mail_parse_context_t *parse_context, int is_attach)
{
	ch_mail_ctnt_t *ctnt;
	char *fname, *fname_tmp;
	ch_fpath_t *fpath = is_attach ? parse_context->attach_fpath : parse_context->content_fpath;
...
static int 
mail_process_content_plain_text(ch_mail_parse_context_t *parse_context,const uint8_t *chunk, uint32_t len,
	MimeDecParseState *state, ch_mail_session_t *s)
{
	...
	if (state->body_begin) {
		...
		ctnt = _get_file_node_for_mail_ctnt_save(sentry->mp,parse_context, 0);
...
static int 
mail_process_content_html(ch_mail_parse_context_t *parse_context,const uint8_t *chunk, uint32_t len,
	MimeDecParseState *state, ch_mail_session_t *s)
{
	ch_mail_ctnt_t *ctnt = NULL;
	ch_mail_session_entry_t *sentry = NULL;
...
static int 
mail_process_attachment(ch_mail_parse_context_t *parse_context,const uint8_t *chunk, uint32_t len, 
	MimeDecParseState *state, ch_mail_session_t *s)
{
	ch_mail_ctnt_t *ctnt = NULL;
	ch_mail_session_entry_t *sentry = NULL;
	...
	if (state->body_begin) {
		/* get file path and append data */
		ctnt = _get_file_node_for_mail_ctnt_save(sentry->mp, parse_context, 1);
...
MailProcessDataChunk(const uint8_t *chunk, uint32_t len, MimeDecParseState *state)
{
	...
	ch_mail_parse_context_t *parse_context = ch_mail_parse_context_get(sentry);

	s = (ch_mail_session_t *) sentry->curr_session;

	if (entity->ctnt_flags & CTNT_IS_ATTACHMENT) {
		ret = mail_process_attachment(parse_context,chunk, len, state, s);
	} else if (entity->ctnt_flags & CTNT_IS_TEXT) {
		ret = mail_process_content_plain_text(parse_context,chunk, len, state, s);
	} else if (entity->ctnt_flags & CTNT_IS_HTML) {
		ret = mail_process_content_html(parse_context,chunk, len, state, s);
	}
