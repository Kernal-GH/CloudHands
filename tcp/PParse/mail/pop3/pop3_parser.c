 ...
 *Change State
 */
static void ChangeState(ch_mail_session_entry_t *msentry, pop3_state_t *state)
{
	...
	if ((state->m_Is_Logined == 1) && (state->m_State == psTransaction)) {
		...
		if (state->m_Cmd == POP3_CMD_QUIT) {
			...
			if (s && s->mime_state) {
				ch_mail_session_store(msentry->sentry, msentry, s);
			}
 ...
 */
static bool 
GetRetcodeAndData(ch_mail_session_entry_t *msentry, 
	ch_mail_curr_line_t *cur_line, pop3_state_t *state)
{
	...
	if (state->m_RetCode == POP3_RET_OK) {
        /* Change m_State */
        ChangeState(msentry, state);
    }
    return true;
}

static bool 
ParseAnswer(ch_mail_session_entry_t *msentry, 
	ch_mail_curr_line_t *cur_line, pop3_state_t *state)
{
	if (!GetRetcodeAndData(msentry, cur_line, state)) {
		return false;
	}
...
static bool 
ParseData(ch_mail_session_entry_t *msentry, 
	ch_mail_curr_line_t *cur_line, pop3_state_t *state)
{   
	/* unused */
	state = state;
...
static bool 
handle_server_data(ch_mail_session_entry_t *msentry, ch_mail_curr_line_t *cur_line)
{
	pop3_state_t *state = (pop3_state_t *) msentry->priv_data;

	if (state->m_State == psData) {
		return ParseData(msentry, cur_line, state); 
	} else if (state->m_State == psAuthorization) {
		return ParseAnswer(msentry, cur_line, state);
	} else if (state->m_State == psTransaction || \
		state->m_State == psUpdate) {
		return ParseAnswer(msentry, cur_line, state);
	}
...
static int pop3_parse(int dir,ch_mail_session_entry_t *msentry, uint8_t *input, uint32_t input_len)
{
	...
	if (dir == 0) {
		...
		{
			if (handle_server_data(msentry, &cur_line) == false) {
				return PARSE_BREAK;
			}
		}
	}
...
int mail_pop3_request_parse(ch_mail_session_entry_t *msentry, void *data,size_t dlen)
{
	...
	return pop3_parse(0, msentry, data, dlen);
}

int mail_pop3_response_parse(ch_mail_session_entry_t *msentry, void *data, size_t dlen)
{
	if (msentry->priv_data == NULL) {
		msentry->priv_data = ch_mail_pop3_state_create(msentry->mp);
	...
	return pop3_parse(1, msentry, data, dlen);
}
