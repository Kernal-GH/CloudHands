...
static int 
_http_body_file_create(ch_http_parse_context_t *http_parse_context,ch_http_session_t *session,int is_req){

	ch_http_session_body_t *body;
	char *fname,*fname_tmp;
	ch_fpath_t *fpath = is_req?http_parse_context->req_fpath:http_parse_context->res_fpath;
...
static int 
_http_body_parse(ch_http_parse_context_t *http_parse_context,ch_http_session_t *session,ch_pp_data_input_t *din,int is_req){

	if(NEED_CREATE_FILE_BODY(session,is_req)){
	
		if(_http_body_file_create(http_parse_context,session,is_req)){
...
static int 
do_http_request_parse(ch_session_entry_t *sentry,void *data,size_t dlen){
	...
	while(ch_pp_din_has_data(din,0)){
		...
		case PARSE_PHASE_REQ_BODY:
			...
			if(_http_body_parse(http_parse_context,session,din,1)){
...
static void _http_session_store(ch_session_entry_t *sentry,ch_http_session_entry_t *hsentry,ch_http_session_t *session){

	ch_http_sentry_session_remove(hsentry,session);
	
	if(ch_session_store_put(sentry,(void*)session)){
...
static int 
do_http_response_parse(ch_session_entry_t *sentry,void *data,size_t dlen){
	...
	while(ch_pp_din_has_data(din,0)){
		...
		case PARSE_PHASE_DONE:
			...
			_http_session_store(sentry,hsentry,session);
			session = ch_http_sentry_session_get(hsentry,0);
			if(session == NULL){
				/* no request??? */
				ch_log(CH_LOG_ERR,"Cannot find http request for this response data!");
		...
		case PARSE_PHASE_RES_BODY:
			...
			if(_http_body_parse(http_parse_context,session,din,0)){
