...
#define is_end(ct) ((ct)->ctype==NULL&&(ct)->ext_name == NULL)
#define is_empty(str) (str==NULL||strlen(str)==0)

static int is_contains(const char *target,const char *match){
...
int ch_http_ctype_is_match(const char *ctype,const char *ext_name){

	size_t ext_name_len = 0;
	int i = 0;
	ch_http_ctype_t *ct;

	if(is_empty(ctype)||is_empty(ext_name))
		return 0;
	...
	while(1){
		...
		if(memcmp(ct->ext_name,ext_name,ext_name_len))
			continue;
