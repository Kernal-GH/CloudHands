 ...
 * Last Modified: 2018-06-12 17:43:55
 */
...
static void _rdata_cert_dump(ch_dns_rdata_t *rdata,FILE *fp,void *priv_data ch_unused){
	...
	if(cert->cert_len){
		...
		decode = malloc(cert->cert_len+1);
		if(decode){
		
			ch_decode_base64_ext(decode,(const unsigned char*)cert->cert,cert->cert_len);
		}

	}

	fprintf(fp,"rdata.cert.cert:%s\n",decode?decode:"");
	if(decode){
