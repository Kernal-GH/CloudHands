...
ch_app_context_t * ch_app_context_tcp_create(ch_pool_t *mp,const char *cfname){

    const char * msg = NULL;
    ch_app_context_tcp_t *app_tcp_context = (ch_app_context_tcp_t*)ch_palloc(mp,sizeof(ch_app_context_tcp_t));

    app_context_tcp_init(app_tcp_context);

    msg = ch_process_command_config(app_context_tcp_directives,(void*)app_tcp_context,mp,mp,cfname);
    ...
    ch_app_context_init((ch_app_context_t*)app_tcp_context,mp);
