...
static int test_assemble_fragment_push(void){
    ...
    CH_TEST_ASSERT(df_count==0&&dfs_len==0,
            ...
            (unsigned long)df_count,(unsigned long)dfs_len);
    ...
    CH_TEST_ASSERT(df_count == 1&&dfs_len == tdf1.dlen,
            "Push a data fragment:Data fragment's count[%d,%lu],size[%lu,%lu]",1,(unsigned long)df_count,(unsigned long)tdf1.dlen,(unsigned long)dfs_len);
    ...
    CH_TEST_ASSERT(df_count == 1&&dfs_len == tdf2.dlen,
            "After push a data fragment[count=[%d,%lu,must eq],size=[%lu,%lu must eq]",1,(unsigned long)df_count,(unsigned long)tdf2.dlen,(unsigned long)dfs_len);
    ...
    ch_assemble_fragment_info_get(as_frag_ptr,&df_count,&dfs_len);
    CH_TEST_ASSERT(df_count == 1&&dfs_len == tdf2.dlen,
            "After pop an unorder, a data fragment[count=[%d,%lu,must eq],size=[%lu,%lu must eq]",1,(unsigned long)df_count,(unsigned long)tdf2.dlen,(unsigned long)dfs_len);
